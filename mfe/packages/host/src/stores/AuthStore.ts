...
export class AuthStore {
  ...
  logout() {
    ...
    runInAction(() => { this.token = null; this.user = null; });
  }

  async init() {
    const storedToken = localStorage.getItem('token');
    const storedRefresh = localStorage.getItem('refreshToken');
    if (!storedToken && !storedRefresh) { runInAction(() => { this.loading = false; }); return; }
    try {
      const res = await getMe(storedToken ?? '');
      runInAction(() => { this.user = res.data.data; this.token = storedToken; });
    } catch {
      if (storedRefresh) {
        try {
          ...
          runInAction(() => { this.token = accessToken; this.user = me.data.data; });
        } catch {
